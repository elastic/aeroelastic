(() => {
  ...
  const initialShapes = [
    {key: 'line1', shape: 'line', x0: 300, y0: 200, x1: 900, y1: 100, z: 5, rotation: 0, color: 'grey'},
  ...
  const nextLine = (down, dragInProgress, hoveredShape, x0, y0, x1, y1, s) => {
    ...
    const newY = beingDragged ? y1 + grabOffsetY : y
    console.log(newX)
    const length = vectorLength(s.x1 - s.x0, s.y1 - s.y0)
    const result = Object.assign({}, s, {
      x: snapToGrid(newX),
      y: snapToGrid(newY),
      x0: snapToGrid(newX),
      y0: snapToGrid(newY),
      x1: snapToGrid(newX) + s.x1 - s.x0,
      y1: snapToGrid(newY) + s.y1 - s.y0,
      unconstrainedX: newX,
      unconstrainedY: newY,
