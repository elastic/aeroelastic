(() => {
  ...
  const initialShapes = [
    {key: 'line1', shape: 'line', x0: 100, y0: 150, x1: 1600, y1: 150, z: 5, rotation: 0, color: 'grey'},
    {key: 'line2', shape: 'line', x0: 100, y0: 550, x1: 1600, y1: 550, z: 5, rotation: 0, color: 'grey'},
  ...
  const currentShapes = xl.lift(function (primedShapes, cursor, dragStartCandidate, {x0, y0, x1, y1, down}) {
    ...
    if(releaseHappened && droppedShape && droppedShape.shape === 'rectangle' && !droppedShape.yConstraint) {
      ...
      for(let i = 0; i < lines.length; i++) {
        ...
        if(dist < closestSnappableLineDistance && dist <= snapDistance && isHorizontal(line)) {
          closestSnappableLine = line
          closestSnappableLineDistance = dist
        }
      }
      if(closestSnappableLine) {
        previousShapeState.find(s => s.key === droppedShape.key).yConstraint = closestSnappableLine.key
      }
    }
